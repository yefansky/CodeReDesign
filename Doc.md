...
使用示例
1. 生成 CVB 文件
打开 VSCode，右键点击工作区，选择 CodeReDesign: Generate CVB File。

选择需要重构的代码文件。

输入重构需求（如“将所有鼠标事件处理代码移动到一个文件中”）。

插件会生成一个 .cvb 文件，并保存到临时目录。

2. 上传 CVB 文件并调用 API
右键点击工作区，选择 CodeReDesign: Upload CVB and Call API。

选择需要上传的 .cvb 文件。

输入提示词（如“重构代码以提高可读性”）。

插件会调用 DeepSeek API，并将返回的 CVB 内容保存为新文件。

3. 查看版本历史
在临时目录中查看所有生成的 .cvb 文件。
